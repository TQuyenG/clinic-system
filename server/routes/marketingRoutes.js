...
router.get('/events/stats', authMiddleware, roleMiddleware('marketing:manage_events', ['admin']), marketingController.getEventStats);
router.get('/events/export', authMiddleware, roleMiddleware('marketing:manage_events', ['admin']), marketingController.exportEvents);
...
router.put('/events/:id/status', roleMiddleware('marketing:manage_events', ['admin']), marketingController.updateEventWorkflowStatus);
...
// ✅ Thêm 2 route mới:
router.put('/promotions/:id/toggle', roleMiddleware('marketing:manage_promotions', ['admin']), marketingController.togglePromotion);
...
// ✅ Wheel Event routes
router.get('/wheel-events',           authMiddleware, roleMiddleware('marketing:manage_promotions', ['admin']), marketingController.getWheelEvents);
router.post('/wheel-events',          authMiddleware, roleMiddleware('marketing:manage_promotions', ['admin']), marketingController.createWheelEvent);
router.put('/wheel-events/:id',       authMiddleware, roleMiddleware('marketing:manage_promotions', ['admin']), marketingController.updateWheelEvent);
router.put('/wheel-events/:id/toggle',authMiddleware, roleMiddleware('marketing:manage_promotions', ['admin']), marketingController.toggleWheelEvent);
router.delete('/wheel-events/:id',    authMiddleware, roleMiddleware('marketing:manage_promotions', ['admin']), marketingController.deleteWheelEvent);
router.get('/wheel-events/winners',   authMiddleware, roleMiddleware('marketing:manage_promotions', ['admin']), marketingController.getWinners);
...
router.post('/events/checkin', roleMiddleware('marketing:manage_events', ['admin']), marketingController.checkInByQR);
router.get('/events/:event_id/registrations', roleMiddleware('marketing:manage_events', ['admin']), marketingController.getEventRegistrations);

// ✅ ROUTE MỚI CHO GIAI ĐOẠN 2
router.post('/events/distribute-gift', roleMiddleware('marketing:manage_events', ['admin']), marketingController.distributeGift);
router.get('/events/:event_id/command-center', roleMiddleware('marketing:manage_events', ['admin']), marketingController.getCommandCenterStats);

// ✅ API CHO CẤU HÌNH ĐIỂM DANH & VÒNG QUAY (ĐỔI ĐIỂM)
router.get('/loyalty-config', authMiddleware, marketingController.getLoyaltyConfig);
router.put('/loyalty-config', authMiddleware, marketingController.updateLoyaltyConfig);
