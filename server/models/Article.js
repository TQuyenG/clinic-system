...
module.exports = (sequelize) => {
  ...
  const Article = sequelize.define('Article', {
    ...
    // === CẬP NHẬT: DUYỆT CHUYÊN MÔN ===
    medical_reviewer_id: {
      type: DataTypes.BIGINT,
      allowNull: true,
      comment: 'ID của Bác sĩ được tag để duyệt bài'
    },
    medical_review_status: {
      type: DataTypes.ENUM('none', 'pending', 'approved', 'rejected'),
      defaultValue: 'none',
      comment: 'Trạng thái duyệt y tế bởi bác sĩ'
    },
    medical_note: {
      type: DataTypes.TEXT,
      allowNull: true,
      comment: 'Ghi chú/Góp ý của bác sĩ duyệt bài'
    },
  ...
  Article.associate = (models) => {
    ...
    // Liên kết với Bác sĩ duyệt bài
    Article.belongsTo(models.User, {
      foreignKey: 'medical_reviewer_id',
      as: 'medicalReviewer'
    });
  };
