...
        // ========== KẾT THÚC THÊM MỚI ==========
        ...
        // --- [THÊM MỚI] LOGIC CHO IT ADMIN ---
        
        // 1. Admin tham gia giám sát (Monitor Join)
        case 'admin_monitor_join':
          // Admin tham gia "ẩn danh", không thông báo user_joined cho bác sĩ/bệnh nhân
          isAdminMonitor = true;
          currentConsultationId = payload.consultation_id;
          ...
          console.log(`🛡️ ADMIN ${userId} đang giám sát phòng ${currentConsultationId}`);
          break;

        // 2. Admin gửi lệnh can thiệp (Force Action)
        case 'admin_action':
          // Chỉ xử lý nếu user thực sự là Admin/Staff (nên check thêm DB role ở đây nếu cần bảo mật cao)
          const { action, target_room } = payload;
          console.log(`⚠️ ADMIN ACTION: ${action} lên phòng ${target_room}`);

          if (action === 'force_reconnect') {
            // Lệnh: Buộc client tải lại socket
            sendSystemCommand(target_room, 'force_reconnect');
          } 
          else if (action === 'restart_signaling') {
            // Lệnh: Khởi động lại quy trình Video Call
            sendSystemCommand(target_room, 'restart_signaling');
          }
          else if (action === 'reload_history') {
             // Lệnh: Tải lại tin nhắn (Client tự fetch API)
