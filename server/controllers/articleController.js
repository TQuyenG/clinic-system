 ...
 * 
 * PHÂN QUYỀN MỚI:
 * - Admin: Thấy tất cả bài viết TRỪĐỀ #1: Admin không thấy draft của tác giả khác
 * - Staff/Doctor: Chỉ thấy bài viết của mình
 */
exports.getArticles = async (req, res) => {
  try {
    ...
    // ===== PHÂN QUYỀN MỚI =====
    if (user.role !== 'admin') {
      // Tác giả (Staff/Doctor): Chỉ thấy bài viết của mình
      where.author_id = user.id;
    } else {
      // Admin: MẶC ĐỊNH KHÔNG THẤY DRAFT CỦA NGƯỜI KHÁC
      // Chỉ thấy:
      // 1. Tất cả bài viết KHÔNG phải draft
      // 2. Draft của chính admin
      where[Op.or] = [
        { status: { [Op.ne]: 'draft' } }, // Tất cả bài không phải draft
        { 
          [Op.and]: [
            { status: 'draft' }, 
            { author_id: user.id } 
      ...
      // Nếu admin chọn tab "Nháp", chỉ hiện draft của chính admin
      if (status === 'draft') {
        delete where[Op.or]; // Xóa điều kiện or
        where.status = 'draft';
        where.author_id = user.id;
      }
    }
    ...
    if (status && !where.status && status !== 'draft') {
      where.status = status;
    }
    ...
    let statsQuery = { author_id: user.id };
    if (user.role === 'admin') {
      // Admin: Đếm tất cả TRỪĐỀ #1: Draft của người khác không đếm
      statsQuery = {};
    }
...
exports.createArticle = async (req, res) => {
  ...
  if (category.category_type === 'thuoc') {
    ...
    if (!isDraft) {
      if (isAdminDirectPublish && req.user.role === 'admin') {
        // Admin publish trực tiếp: tạo history approve, không gửi thông báo cho admin khác
        ...
        // Gửi phê duyệt: tạo history submit và gửi thông báo cho admin
