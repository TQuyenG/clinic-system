...
exports.getMyMedicalRecords = async (req, res) => {
  try {
    ...
    if (role === 'doctor') {
      ...
      whereCondition = { doctor_id: doctor.id };
    } else {
      // patient / admin / staff
      ...
      if (!patient) {
        // Trả về mảng rỗng thay vì 404 để frontend hiển thị "chưa có hồ sơ"
        return res.status(200).json({ success: true, data: [] });
      }
      whereCondition = { patient_id: patient.id };
    }

    // 2. Lấy tất cả hồ sơ y tế theo điều kiện
