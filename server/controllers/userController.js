...
/**
 * LẤY DANH SÁCH USER THEO ROLE (DÙNG CHO SCHEDULE FORM)
...
exports.getUsersByRole = async (req, res) => {
  try {
    ...
    if (!role) {
      return res.status(400).json({
        success: false,
        message: 'Tham số role là bắt buộc'
      });
    }
...
exports.getMyRoleInfo = async (req, res) => {
  try {
    ...
    if (user.role === 'patient' && user.Patient) {
      ...
      // Parse medical_history từ JSON string (nếu lưu dạng string) hoặc object
      ...
      // Gán lại object đã parse để frontend dùng
      roleData.setDataValue('medical_history', healthInfo);
    } 
    else if (user.role === 'doctor') roleData = user.Doctor;
    else if (user.role === 'staff') roleData = user.Staff;
