...
exports.getConsultationReports = async (req, res) => {
  try {
    ...
    const reports = await ConsultationReport.findAndCountAll({
      where,
      include: [
        { model: Consultation, as: 'consultation', attributes: ['id', 'consultation_code'] },
...
exports.resolveReport = async (req, res) => {
  try {
    ...
    const report = await ConsultationReport.findByPk(reportId);
    if (!report) {
      return res.status(404).json({
        success: false,
    ...
    await Notification.create({
      ...
      related_type: 'consultation_report'
    });
