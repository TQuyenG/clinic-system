...
/**
  ...
  handleMessage: function(data) {
    ...
    // --- [THÊM MỚI] XỬ LÝ LỆNH HỆ THỐNG TỪ ADMIN ---
    if (type === 'system_command') {
      console.warn('⚡ NHẬN LỆNH HỆ THỐNG:', payload.command);
      
      switch (payload.command) {
        case 'force_reconnect':
          // Đóng socket và kết nối lại
          this.disconnect();
          setTimeout(() => {
             // userId lấy từ localStorage hoặc context
             const user = JSON.parse(localStorage.getItem('user'));
             if(user) this.connect(user.id, payload.consultation_id);
          }, 1000);
          // Emit event để UI hiển thị thông báo "Đang kết nối lại..."
        ...
        case 'restart_signaling':
          // Emit event để VideoService bắt được và restart
