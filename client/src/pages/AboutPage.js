...
const AboutPage = () => {
  ...
  useEffect(() => {
    const fetchAboutData = async () => {
      try {
        const response = await axios.get('http://localhost:3002/api/settings/about');
        if (response.data) {
    ...
    const fetchDoctors = async () => {
      try {
        const response = await fetch('http://localhost:3002/api/users/doctors?limit=6&random=true');
        const data = await response.json();
