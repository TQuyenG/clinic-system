...
const ServiceManagementPage = () => {
  ...
  const hasPermission = (module, permission) => {
    ...
    if (!user?.role_info?.permissions) {
      console.log('⚠️ [ServiceManagementPage] No permissions found in user.role_info.permissions');
      console.log('👤 User object:', user);
      return false;
    }
    const modulePerms = user.role_info.permissions[module];
    if (!modulePerms) {
      console.log(`⚠️ [ServiceManagementPage] Module '${module}' not found in permissions`);
      return false;
    }
    const hasAccess = Array.isArray(modulePerms) ? modulePerms.includes(permission) : false;
    console.log(`✓ [ServiceManagementPage] Check ${module}.${permission}:`, hasAccess);
    return hasAccess;
  };

  // ✅ Check nếu có BẤT KỲ quyền nào trong module services
  const canAccessModule = () => {
    if (isAdmin) return true;
    if (!user?.role_info?.permissions) return false;
    const modulePerms = user.role_info.permissions['services'];
    if (!modulePerms) return false;
    // Có array permissions và có ít nhất 1 phần tử
    return Array.isArray(modulePerms) && modulePerms.length > 0;
  };

  const canView = canAccessModule(); // Thay đổi logic: có quyền module là được view
  ...
  // Debug logging
  useEffect(() => {
    console.log('🔐 [ServiceManagementPage] Permissions check:');
    console.log('  - User:', user?.username);
    console.log('  - Role:', user?.role);
    console.log('  - Is Admin:', isAdmin);
    console.log('  - Permissions:', user?.role_info?.permissions);
    console.log('  - Services permissions:', user?.role_info?.permissions?.services);
    console.log('  - Can Access Module:', canView);
  ...
  // ✅ CHECK PERMISSION - Đặt sau tất cả hooks
  if (!canView && !isAdmin) {
    return (
      <div style={{ padding: '50px', textAlign: 'center' }}>
        <h2>⛔ Bạn không có quyền truy cập chức năng này</h2>
        <p>Module: <strong>services</strong></p>
        <p>Bạn cần có ít nhất một quyền trong module này để truy cập.</p>
        <p>Vui lòng liên hệ quản trị viên để được cấp quyền.</p>
        <button onClick={() => window.history.back()} style={{ marginTop: '20px', padding: '10px 20px' }}>
          ← Quay lại
        </button>
      </div>
    );
