...
import { 
  ...
  FaFileExport, FaSpinner, FaFilter
} from 'react-icons/fa';
...
const EntityManagementPage = ({ entityType }) => {
  ...
  const fetchSuggestions = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/articles/suggestions?${params}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
                      ...
                      {suggestions.map((suggestion, index) => (
                        <tr key={suggestion.id}>
                          ...
                          <td>
                            <div className="entity-mgmt-table-actions">
                              <button
                                className={`entity-mgmt-btn entity-mgmt-btn-sm ${isAdmin && suggestion.status === 'pending' ? 'entity-mgmt-btn-primary' : 'entity-mgmt-btn-secondary'}`}
                                onClick={() => {
                                  setSuggestionMode('review');
                                  setSelectedSuggestion(suggestion);
                                  setShowSuggestionModal(true);
                                }}
                                title={isAdmin && suggestion.status === 'pending' ? 'Phê duyệt' : 'Xem chi tiết'}
                              >
                                {isAdmin && suggestion.status === 'pending' ? <FaCheck /> : <FaEye />}
                              </button>
                            </div>
                          </td>
                        </tr>
