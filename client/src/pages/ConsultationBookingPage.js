...
const ConsultationBookingPage = () => {
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      ...
      // 1. Gọi API từ specialtyService (Sửa từ userService -> specialtyService)
      const response = await specialtyService.getPublicSpecialties();
      
      // 2. Log để debug (xem cấu trúc thực tế trả về là gì)
      console.log("API Specialties Response:", response.data);

      // 3. Xử lý dữ liệu an toàn: Tìm mảng trong các trường hợp response khác nhau
      const rawData = response.data;
      let specialtiesArray = [];

      if (Array.isArray(rawData)) {
        specialtiesArray = rawData;
      } else if (rawData && Array.isArray(rawData.data)) {
        specialtiesArray = rawData.data;
      } else if (rawData && Array.isArray(rawData.specialties)) {
        specialtiesArray = rawData.specialties;
      }

      // 4. Set state (Đảm bảo luôn là mảng)
      setSpecialties(specialtiesArray);
      
      // 5. Fetch dữ liệu bác sĩ nếu có ID
      if (doctorId) {
      ...
      setSpecialties([]); // Fallback về mảng rỗng nếu lỗi
    } finally {
      setLoading(false);
    }
  }, [doctorId]);
