...
const UsersPage = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:3002/api/users/search', {
        params,
        ...axiosConfig
      });
  ...
  const handleToggleStatus = async (userId, currentStatus) => {
    ...
    try {
      await axios.put(
        `http://localhost:3002/api/users/${userId}/toggle-status`,
    ...
    if (action === 'delete') {
      ...
      try {
        await Promise.all(selectedUsers.map(id => 
          axios.delete(`http://localhost:3002/api/users/${id}`, axiosConfig)
        ));
      ...
      try {
        await Promise.all(selectedUsers.map(id => 
          axios.put(`http://localhost:3002/api/users/${id}/toggle-status`, { is_active: false }, axiosConfig)
        ));
      ...
      try {
        await Promise.all(selectedUsers.map(id => 
          axios.put(`http://localhost:3002/api/users/${id}/toggle-status`, { is_active: true }, axiosConfig)
        ));
  ...
  const handleBulkPasswordReset = async () => {
    ...
    try {
      ...
      await Promise.all(selectedUsers.map(async (id) => {
        await axios.put(
          `http://localhost:3002/api/users/${id}/reset-password-admin`,
  ...
  const handleDelete = async (userId) => {
    ...
    try {
      await axios.delete(`http://localhost:3002/api/users/${userId}`, axiosConfig);
      alert('Xóa thành công');
      fetchUsers();
    } catch (error) {
      alert('Lỗi: ' + (error.response?.data?.message || error.message));
