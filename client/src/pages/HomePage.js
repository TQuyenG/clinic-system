...
const HomePage = () => {
    ...
    const fetchSpecialties = async () => {
      try {
        ...
        if (data.success && data.specialties) {
          // Lấy icon từ database, nếu không có thì dùng mặc định
          const specialtiesWithIcons = data.specialties.map(spec => ({
            ...spec,
            iconName: spec.icon || 'FaStethoscope'
          }));
  ...
  try {
    ...
    const fetchHospitalServices = async () => {
      try {
        ...
        if (response.data.success && response.data.data) {
          setHospitalServices(response.data.data.slice(0, 6));
        } else {
          setHospitalServices([]);
        }
      } catch (error) {
        console.error('Lỗi khi lấy dữ liệu dịch vụ khám:', error);
        setHospitalServices([]);
      }
    };
    ...
    const fetchConsultationServices = async () => {
      try {
        ...
        console.error('Lỗi khi lấy dữ liệu tư vấn:', error);
        setConsultationServices([]);
      }
    };
  ...
  useEffect(() => {
    ...
    // Tạo interval để tự động chuyển slide - mỗi 6 giây
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Build query params to pass to booking page
      ...
      <section className="homepage-banner-slider">
        {(homeSettings.bannerSlides || []).map((slide, index) => {
          ...
          return (
            <div
              key={index}
              className={`homepage-banner-slide ${isActive ? 'active' : isPrevious ? 'previous' : ''}`}
          ...
          <div className="homepage-container">
            ...
            <div className="homepage-services-grid">
              {hospitalServices.map((service, index) => (
                <div 
                  key={service.id} 
                  className={`homepage-service-card ${isVisible['hospital-services'] ? 'fade-in' : ''}`}
                  style={{ animationDelay: `${index * 0.1}s` }}
                >
                    ...
                    {service.image && service.image.trim() ? (
                      <>
                        ...
                        <div className="homepage-service-overlay-info">
                          <div className="homepage-service-price-overlay">
                            {service.price ? service.price.toLocaleString('vi-VN') + 'đ' : 'Liên hệ'}
                          </div>
                          {service.duration && (
                            <div className="homepage-service-duration-overlay">
                      ...
                      <div className="homepage-service-image-placeholder">
                        <FaIcons.FaStethoscope className="placeholder-icon" />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="homepage-service-content">
                    ...
                    <button 
                      className="homepage-service-btn"
                      onClick={() => navigate(`/dich-vu/${service.id}`)}
                    >
                      Xem chi tiết
                      <FaIcons.FaArrowRight />
                    </button>
            ...
            {hospitalServices.length > 9 && (
              <div className="homepage-section-footer">
                <Link to="/dich-vu" className="homepage-btn homepage-btn-outline">
                  Xem tất cả dịch vụ
                  <FaIcons.FaArrowRight />
                </Link>
              </div>
          ...
          <div className="homepage-container">
            ...
            <div className="homepage-services-grid">
              {consultationServices.map((service, index) => {
                // Support both field names từ service và consultationPricing
                const packageName = service.name || service.package_name;
                const packageDesc = service.description;
                const packagePrice = service.price;
                const packageDuration = service.duration || service.duration_minutes;
                const packageType = service.package_type;
                
                // Map icon dựa vào package_type hoặc icon
                let iconClass = 'FaComments';
                if (service.icon && service.icon.startsWith('Fa')) {
                  iconClass = service.icon;
                } else if (packageType === 'chat') {
                  iconClass = 'FaComments';
                } else if (packageType === 'video') {
                  iconClass = 'FaVideo';
                } else if (packageType === 'offline') {
                  iconClass = 'FaStethoscope';
                }
                
                return (
                  <div 
                    key={service.id} 
                    className={`homepage-service-card ${isVisible['consultation-services'] ? 'fade-in' : ''}`}
                    style={{ animationDelay: `${index * 0.1}s` }}
                  >
                    <div className="homepage-service-image-wrapper">
                      <div className="homepage-service-image-placeholder">
                        {iconClass && iconClass.startsWith('Fa') ? (
                          React.createElement(FaIcons[iconClass], { className: 'placeholder-icon' })
                        ) : (
                          <FaIcons.FaComments className="placeholder-icon" />
                        )}
                      </div>
                      <div className="homepage-service-overlay-info">
                        <div className="homepage-service-price-overlay">
                          {packagePrice === 0 ? 'Miễn phí' : (packagePrice ? packagePrice.toLocaleString('vi-VN') + 'đ' : 'Liên hệ')}
                        </div>
                        {packageDuration && (
                          <div className="homepage-service-duration-overlay">
                    ...
                    <div className="homepage-service-content">
                      <h3>{packageName}</h3>
                      <p className="homepage-service-description">{packageDesc}</p>
                      
                      <button 
                        className="homepage-service-btn homepage-service-btn-consultation"
                        onClick={() => navigate(`/tu-van?type=${service.id}`)}
                      >
                        Đặt ngay
                        <FaIcons.FaArrowRight />
                      </button>
