            ...
            <table className="admin-appt-page-table">
              <thead>
                <tr>
                  ...
                  <th>Bác sĩ</th>
                  <th>Ngày & Giờ</th>
                  <th>Thanh toán</th> {/* Cột mới thêm vào đúng vị trí */}
                  <th>Trạng thái</th>
                      ...
                      <React.Fragment key={apt.id}>
                        ...
                        <tr className={isExpanded ? 'row-expanded' : ''}>
                          ...
                          <td data-label="Thanh toán">
                             <div style={{display:'flex', flexDirection:'column', gap:'4px'}}>
                                <strong style={{color:'#dc2626'}}>
                                   {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(apt.Service?.price || 0)}
                                </strong>
                                { (apt.payment_status === 'paid' || apt.Payment?.status === 'paid') ? (
                                    <span className="admin-appt-page-status-badge status-completed" style={{fontSize:'0.7rem', padding:'2px 6px'}}>
                                        <FaCheckCircle/> Đã thanh toán
                                    </span>
                                ) : (
                                    <span className="admin-appt-page-status-badge status-pending" style={{fontSize:'0.7rem', padding:'2px 6px'}}>
                                        Chưa thanh toán
                                    </span>
                                )}
                             </div>
                          ...
                          <td data-label="Trạng thái">
                            ...
                            {(() => {
                                const isPaid = apt.payment_status === 'paid' || apt.Payment?.status === 'paid';
                                if (isPaid && apt.status === 'pending') {
                                    return <span className="admin-appt-page-status-badge status-confirmed"><FaCheckCircle /> Đã xác nhận</span>;
                                }
                          ...
                          <td data-label="Thao tác">
                            <div className="admin-appt-page-action-buttons">
                              ...
                              {/* --- LOGIC NÚT BẤM SỬA LẠI --- */}
                              {(() => {
                                  const isPaid = apt.payment_status === 'paid' || apt.Payment?.status === 'paid';
                                  
                                  return (
                                    <>
                                      {/* Chỉ hiện nút Duyệt khi CHƯA thanh toán và đang chờ */}
                                      ...
                                      {/* Hiện nút Hoàn thành khi Đã xác nhận HOẶC Đã thanh toán */}
                                      {(apt.status === 'confirmed' || apt.status === 'in_progress' || (apt.status === 'pending' && isPaid)) && (
                                        <button 
                                      ...
                                      {/* Nút Hủy */}
                                      {(apt.status === 'pending' || apt.status === 'confirmed' || (apt.status === 'pending' && isPaid)) && (
                                        <button 
