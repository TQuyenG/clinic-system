...
const { showToast } = useToast();
  ...
  if (id && user) {
    ...
    // Lắng nghe sự kiện restart video từ Admin
    chatService.on('restart_video', async () => {
       console.log('Admin requested video restart');
       const { default: videoService } = await import('../../services/videoService');
       await videoService.restartSession();
    });

    // Lắng nghe lệnh reload lịch sử
    chatService.on('reload_history', () => {
       fetchConsultationData();
    });

    // Polling mỗi 3 giây để đồng bộ tin nhắn realtime
    ...
    switch (data.type) {
      ...
      case 'system_command':
        if (data.payload.command === 'force_reconnect') {
           showToast({ type: 'warning', message: '⚠️ Hệ thống đang tối ưu hóa kết nối. Vui lòng đợi trong giây lát...' });
           window.location.reload(); // Cách đơn giản nhất để refresh toàn bộ trạng thái
        }
        break;
      // ------------------
      default:
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      showToast({ type: 'error', message: 'Không thể gửi tin nhắn. Vui lòng thử lại.' });
    } finally {
      setSending(false);
    }
  };
