...
const MonthView = ({ 
  ...
  return appointments.some(app => 
    ...
    new Date(app.date || app.appointment_date).setHours(0,0,0,0) === checkTime
  );
};
  ...
  const processedEventsByDay = useMemo(() => {
    ...
    appointments.forEach(event => {
        ...
        if (sTime && eTime && appDate) {
            ...
            const dayKey = new Date(appDate).setHours(0,0,0,0);
            if (!eventMap.has(dayKey)) eventMap.set(dayKey, []);
            eventMap.get(dayKey).push(e);
        }
    });
    ...
    return (
      <>
        {eventsWithLayout.map(event => {
          ...
          const userColor = getColorForUser(displayUser?.id || event.user?.id, selectedUsers);
...
const CalendarView = (props) => {
  ...
  const selectedUserIds = (selectedUsers && selectedUsers.length > 0)
    ? new Set(selectedUsers.map(u => u.value))
    : null;

  const matchSelected = (evt) => {
    // evt có thể có user_id hoặc user.id
    if (!selectedUserIds) return true; // không lọc
    if (!evt) return false;
    if (evt.user_id && selectedUserIds.has(evt.user_id)) return true;
    if (evt.user && (evt.user.id && selectedUserIds.has(evt.user.id))) return true;
    // một số object có cấu trúc khác (ví dụ appointment.userId)
    if (evt.userId && selectedUserIds.has(evt.userId)) return true;
    return false;
  };

  const filteredSchedules = selectedUserIds ? schedules.filter(s => matchSelected(s)) : schedules;
  const filteredOvertime = selectedUserIds ? overtimeSchedules.filter(s => matchSelected(s)) : overtimeSchedules;
  const filteredLeaves = selectedUserIds ? leaveRequests.filter(s => matchSelected(s)) : leaveRequests;
  const filteredAppointments = selectedUserIds ? appointments.filter(s => matchSelected(s)) : appointments;
