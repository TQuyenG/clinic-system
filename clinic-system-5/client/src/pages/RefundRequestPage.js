...
const RefundRequestPage = () => {
  ...
  const checkPolicyCompliance = (req) => {
    ...
    if (snapshotPercent !== null && snapshotPercent !== expectedPercent) {
      warning = `⚠️ Chính sách đã thay đổi: Lúc hủy áp dụng ${snapshotPercent}%, hiện tại quy định ${expectedPercent}%`;
    }
                ...
                <tr key={req.id}>
                  ...
                  <td>
                    <span style={{
                      ...
                      border: `1px solid ${req.order_type === 'consultation' ? '#bfdbfe' : '#bbf7d0'}`
                    }}>
                      {req.order_type === 'consultation' ? '💻 Tư vấn' : '🏥 Lịch hẹn'}
                    </span>
                  </td>
                  <td>
                  ...
                  <td style={{maxWidth:'180px'}}>
                    ...
                    {verdict.eligible === false && (
                      <div style={{fontSize:'11px', color:'#b91c1c', background:'#fef2f2', padding:'3px 6px', borderRadius:'4px', border:'1px solid #fecaca', marginBottom:'3px'}}>
                        ❌ {verdict.reason}
                      </div>
                    )}
                    {verdict.eligible === null && (
                      <div style={{fontSize:'11px', color:'#92400e', background:'#fffbeb', padding:'3px 6px', borderRadius:'4px', border:'1px solid #fde68a'}}>
                        ⚠️ {verdict.reason}
                      </div>
                    )}
                    {verdict.warning && (
              ...
              {policyVerdict && selectedReq.status === 'pending' && (
                <div style={{
                  ...
                  <div style={{fontWeight:700, fontSize:'13px', marginBottom:'4px', color: policyVerdict.eligible === true ? '#047857' : policyVerdict.eligible === false ? '#b91c1c' : '#92400e'}}>
                    {policyVerdict.eligible === true ? '✅ Đủ điều kiện hoàn tiền' : policyVerdict.eligible === false ? '❌ Không đủ điều kiện hoàn tiền' : '⚠️ Cần xem xét thủ công'}
                  </div>
                  ...
                  {policyVerdict.eligible && policyVerdict.booking_fee > 0 && (
                    <div style={{
                      ...
                      <div style={{color:'#92400e', fontWeight:600, marginBottom:'2px'}}>💰 Chi tiết số tiền hoàn:</div>
                      ...
                      <div style={{
                        ...
                        ✅ Thực hoàn: <b>{formatCurrency(policyVerdict.actual_refund_amount)}</b>
                      </div>
                      {policyVerdict.actual_refund_amount !== parseFloat(selectedReq.refund_amount) && (
                        <div style={{color:'#d97706', marginTop:'4px', fontStyle:'italic'}}>
                          ⚠️ Số tiền QR hiện tại ({formatCurrency(selectedReq.refund_amount)}) chưa khớp với tính toán chính sách. Nên hoàn đúng: {formatCurrency(policyVerdict.actual_refund_amount)}
                        </div>
                      )}
                    </div>
                  )}
