...
exports.getAuditLogs = async (req, res) => {
  try {
    ...
    if (startDate || endDate) {
      ...
      if (startDate) {
        whereClause.created_at[models.Sequelize.Op.gte] = new Date(startDate);
      }
      if (endDate) {
        const end = new Date(endDate);
        end.setHours(23, 59, 59, 999);
        whereClause.created_at[models.Sequelize.Op.lte] = end;
      }
    }
...
exports.getAuditStats = async (req, res) => {
  try {
    ...
    if (startDate || endDate) {
      whereClause.created_at = {};
      if (startDate) whereClause.created_at[models.Sequelize.Op.gte] = new Date(startDate);
      if (endDate) {
        const end = new Date(endDate);
        end.setHours(23, 59, 59, 999);
        whereClause.created_at[models.Sequelize.Op.lte] = end;
