...
const { sequelize, initializeDatabase, seedData, cleanupDuplicateIndexes } = require('./config/db');
...
// WebSocket Server (resilient startup)
...
function startWebSocketServer() {
  try {
    ...
    server.on('listening', () => {
      console.log(`SUCCESS: WebSocket server đang chạy trên cổng ${WS_PORT}`);
    });
    server.on('error', (err) => {
      if (err && err.code === 'EADDRINUSE') {
        console.warn(`WARN: WS port ${WS_PORT} đang được sử dụng. Bỏ qua khởi tạo WebSocket.`);
      } else {
        console.error('ERROR: WebSocket server error:', err.message);
      }
    });
  } catch (err) {
    console.warn('WARN: Không thể khởi động WebSocket server:', err.message);
  }
}
...
async function startServer() {
  try {
    ...
    await cleanupDuplicateIndexes('specialties', ['name', 'slug']);
