...
const getForumOverview = async (req, res, next) => {
	try {
		// Đếm tổng số câu hỏi đã duyệt
		const totalQuestions = await models.Question.count({
			where: { status: 'approved' }
		});

		// Đếm tổng số câu trả lời (không bị xóa)
		const totalAnswers = await models.Answer.count({
			where: { isDeleted: false }
		});

		// Đếm số topic có câu hỏi (thay vì specialty)
		const topicCount = await models.Question.count({
			where: { 
				status: 'approved',
				topicId: { [Op.ne]: null }
			},
...
module.exports = {
	...
	getAllQuestions: async (req, res, next) => {
		...
		try {
			...
			// ✅ THÊM: Nếu là staff →
			// Nếu là manager CSKH/Content thì xem mọi câu hỏi như admin
			if (req.user && req.user.role === 'staff') {
				const { rank, department } = req.user.role_info || {};
				if (rank === 'manager' && (department === 'support' || department === 'content')) {
					// Không filter topicId, xem mọi câu hỏi
				} else {
					// Lấy tất cả topics mà staff này là moderator
	...
	}			// Nếu là staff → Chỉ xem reports của questions thuộc topic mình được phân công
		if (req.user.role === 'staff') {
			// ✅ FIX: Lấy tất cả topics rồi filter trong code (tránh lỗi JSON query)
			const allTopics = await models.Topic.findAll({
				attributes: ['id', 'moderatorIds']
			});
	...
	createTopic: async (req, res, next) => {
		try {
			const { name, title, description, icon, color, requiresApproval, autoApprove, moderatorIds } = req.body;
	...
	updateTopic: async (req, res, next) => {
		try {
			const { id } = req.params;
			const { name, title, description, icon, color, requiresApproval, autoApprove, isActive, moderatorIds } = req.body;
