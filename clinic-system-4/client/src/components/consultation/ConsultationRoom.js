  ...
  useEffect(() => {
    if (id && user) {
      ...
      chatService.on('restart_video', async () => {
         ...
         // Import videoService dynamically hoặc pass prop
      ...
      chatService.on('reload_history', () => {
         fetchConsultationData(); // Tải lại tin nhắn từ API
      });
    }
    ...
    switch (data.type) {
      ...
      case 'system_command':
        if (data.payload.command === 'force_reconnect') {
           alert('⚠️ Hệ thống đang tối ưu hóa kết nối. Vui lòng đợi trong giây lát...');
