...
const ForumManagementPage = () => {
  ...
  const myTopics = useMemo(() => {
    ...
    // Admin có quyền quản lý tất cả topics
    if (currentUser.role === 'admin') {
      return topics;
    }
    
    // Manager (trưởng phòng) của CSKH hoặc Content có quyền quản lý tất cả topics
    if (currentUser.role === 'staff') {
      ...
      if (rank === 'manager' && (department === 'support' || department === 'content')) {
        return topics; // Trưởng phòng CSKH/Content thấy tất cả
      }
      
      // Staff thường chỉ thấy topics được phân công
  ...
  const questionsToShow = useMemo(() => {
    ...
    let isManagerAllTopics = false;
    if (currentUser?.role === 'staff') {
      const staff = currentUser.staff || currentUser.role_info;
      const rank = staff?.rank;
      const department = staff?.department;
      if (rank === 'manager' && (department === 'support' || department === 'content')) {
        isManagerAllTopics = true;
      }
    }
