...
import {
  ...
  FaExclamationTriangle, FaWallet, FaCreditCard, FaTimes, FaComments
} from 'react-icons/fa';
...
const AppointmentBookingPage = () => {
  ...
  useEffect(() => {
    ...
    // Load from query params if coming from homepage booking form
    ...
    if (queryName || queryPhone || queryEmail || querySpecialty || queryDate) {
      setFormData(prev => ({
        ...prev,
        ...
        // specialty will be handled after services load
      }));
    }
    ...
    const loadAllServices = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          // Handle pre-selected service from URL param
          if (preSelectedServiceId) {
            handleServiceChange(preSelectedServiceId, response.data.data);
          }
          
          // If coming from homepage, try to match specialty to service
          const queriedSpecialty = searchParams.get('specialty');
          if (queriedSpecialty) {
            // Try to find a service matching this specialty
            const matchingService = response.data.data.find(s => s.specialty_id === parseInt(queriedSpecialty));
            if (matchingService) {
              handleServiceChange(matchingService.id, response.data.data);
            }
          }
        }
      } catch (error) {
    ...
    loadAllServices();
  }, [searchParams]); // Include searchParams in dependency
  ...
  setDoctors(normalizeUserList(doctorsData, 'doctor'));
    ...
    <div className="appointment-booking-page">
      <div className="appointment-booking-container">
        ...
        <div className="appointment-booking-page-header">
          <div className="appointment-booking-header-top">
            <button className="appointment-booking-back-btn" onClick={() => navigate(-1)} title="Quay lại">
              ← Quay lại
            </button>
            <button className="appointment-booking-switch-btn" onClick={() => navigate('/dat-lich-tu-van')} title="Đặt lịch tư vấn">
